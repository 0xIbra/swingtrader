#!/usr/bin/env python3
"""
TASK 9: Sequence Window Builder (4H Timeframe)
Construct TCN-ready sliding windows for 4H bars.

Adjusted parameters:
- Sequence length: 42 bars (168 hours = 1 week at 4H)
"""
...
def build_sequences(
    df: pd.DataFrame,
    feature_cols: list,
    seq_len: int = 42,  # 42 bars = 1 week at 4H
    skip_missing: bool = True
) -> dict:
    """
    Build sliding window sequences for TCN model (4H timeframe).

    For each timestamp t with labels:
    - X = features[t-41:t] (shape: [42, feature_dim])
    - y_class = direction[t]
    ...
    Args:
        ...
        seq_len: Sequence length (42 bars = 1 week at 4H)
        skip_missing: Skip windows with missing_flag=1

    Returns:
        Dictionary with X, y_class, y_reg arrays and metadata
    """
    print("\n" + "="*80)
    print(f"Building Sequences (seq_len={seq_len} bars = {seq_len * 4} hours)")
    print("="*80)

    print(f"\nFeatures selected: {len(feature_cols)}")
    print(f"Sequence length: {seq_len} bars ({seq_len * 4} hours = 1 week)")
    ...
    for idx in tqdm(valid_indices, desc="Building sequences"):
        ...
        # Extract features
        X = window[feature_cols].values  # [seq_len, n_features]
    ...
    y_class_mb = y_class.nbytes / (1024 ** 2)
    y_reg_mb = y_reg.nbytes / (1024 ** 2)
    total_mb = x_mb + y_class_mb + y_reg_mb

    print(f"  - X:       {x_mb:8.2f} MB")
    print(f"  - y_class: {y_class_mb:8.2f} MB")
    print(f"  - y_reg:   {y_reg_mb:8.2f} MB")
...
def analyze_sequences(dataset: dict) -> None:
    ...
    constant_features = [feature_names[i] for i, var in enumerate(feature_vars) if var < 1e-10]
    if constant_features:
        print(f"\n  ⚠️  {len(constant_features)} nearly constant features:")
    ...
    print(f"  - First: {timestamps.min()}")
    print(f"  - Last:  {timestamps.max()}")
    print(f"  - Range: {(timestamps.max() - timestamps.min()).days} days")
...
def main():
    ...
    print("="*80)
    print("TASK 9: Sequence Window Builder (4H)")
    print("="*80)
    ...
    feature_categories = {
        ...
        'Economic Events': [c for c in feature_cols if 'event' in c or 'hours_' in c],
        'Sentiment': [c for c in feature_cols if 'sent_' in c],
        'Macro Regime': [c for c in feature_cols if any(m in c for m in ['spx_', 'vix_', 'yield10_', 'gold_', 'oil_', 'dxy_'])],
    ...
    # Save metadata
    metadata_file = str(output_dir / 'sequences_metadata_4h.txt')
    with open(metadata_file, 'w') as f:
        f.write("="*80 + "\n")
        f.write("Sequence Dataset Metadata (4H Timeframe)\n")
        f.write("="*80 + "\n\n")
        ...
        f.write(f"  - Timeframe: 4H\n")
        f.write(f"  - Features: {dataset['n_features']}\n")
        f.write(f"  - Samples: {dataset['n_samples']:,}\n\n")
        f.write(f"Data Shapes:\n")
        f.write(f"  - X: {dataset['X'].shape}\n")
    ...
    print("\n" + "="*80)
    print("✅ TASK 9 COMPLETE (4H)")
    print("="*80)
    print(f"✓ Sequence dataset: {saved_path}")
    print(f"✓ Total sequences: {dataset['n_samples']:,}")
    print(f"✓ Shape: [n={dataset['n_samples']}, seq={dataset['seq_len']}, feat={dataset['n_features']}]")
